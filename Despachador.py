...
ventana = Tk()
canvas = Canvas(ventana, borderwidth=0, background="#ffffff")
frame = Frame(canvas, background="#ffffff")
vsb = Scrollbar(ventana, orient="vertical", command=canvas.yview)
...
def printTable(processList, mp):
    ...
    microID = Text(
        frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana"
    )
    name = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    time = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    tcc = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    tvc = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    tb = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    tt = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    ti = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    tf = Text(frame, height=1, width=10, bg="#85AFD5", fg="#0B2332", font="verdana")
    ...
    for i in range(mp):
        ...
        microTitle = Text(frame, height=1, width=20, font="verdana")
...
def retrieve_input():
    ...
    if mp > 0 and tcc >= 0 and tb >= 0 and quantum > 0:
        ...
        mensaje = Text(frame, height=1, width=10)
        mensaje.insert(END, "VALORES")
        mensaje.configure(state=DISABLED)
        mensaje2 = Text(frame, height=1, width=10)
        mensaje2.insert(END, "NO VALIDOS")
        mensaje2.configure(state=DISABLED)
        mensaje.grid(row=5, padx=5, pady=5)
        mensaje2.grid(row=6, padx=5, pady=5)


# interfaz inicial
tccBox = Text(frame, height=2, width=10)
tccBoxText = Text(frame, height=2, width=10)
tbBox = Text(frame, height=2, width=10)
tbBoxText = Text(frame, height=2, width=10)
quantumBox = Text(frame, height=2, width=10)
quantumBoxText = Text(frame, height=2, width=10)
mpBox = Text(frame, height=2, width=10)
mpBoxText = Text(frame, height=2, width=10)
buttonCommit = Button(
    frame, height=1, width=10, text="Despachar", command=lambda: retrieve_input()
)
